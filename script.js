...
function dialogBox(x, y, text) {
    ...
    newDiv.style.backgroundColor = "#888";
    ...
    if ( x  - (newDiv.clientWidth / 2) <= 0 ) {posX = 0} // left
    if ( x  + (newDiv.clientWidth / 2) >= window.innerWidth ) {posX = window.innerWidth - newDiv.clientWidth } // right
    if ( y  - (newDiv.clientHeight / 2) <= 0 ) {posY = 0} // top
    if ( y  + (newDiv.clientHeight / 2) >= window.innerHeight ) {posY = window.innerHeight - newDiv.clientHeight } // bottom
